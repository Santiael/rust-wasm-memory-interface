...
class Allocator {
  constructor(memory, allocateOnHost, deallocateOnHost) {
    this._memory = memory;
    this._allocateOnHost = allocateOnHost;
    this._deallocateOnHost = deallocateOnHost;

    this._textEncoder = new TextEncoder();

    this._allocateByType = {
      'number': (jsNumber) => {
        const pointer = this._allocateOnHost(NUMBER_BYTES);
      ...
      'boolean': (jsBoolean) => {
        const pointer = this._allocateOnHost(BOOLEAN_BYTES);
      ...
      'string': (jsString) => {
        ...
        const pointer = this._allocateOnHost(stringSize);
  ...
  free({ pointer, size }) {
    try {
      this._deallocateOnHost(pointer, size);
